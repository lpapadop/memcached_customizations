...
#ifdef PIN_THREADS_TO_CORES
    cpu_set_t set;
    int cpu_to_be_pinned = ((LIBEVENT_THREAD*)arg)->cpu_to_be_pinned;
	CPU_ZERO(&set);
	// set thread with thread_id to cpu set
	CPU_SET(cpu_to_be_pinned, &set);
	if (sched_setaffinity(syscall(__NR_gettid), sizeof(cpu_set_t), &set)) {
		exit(1);
	}
	printf("thread %d pinned at cpu core: %d\n", (int)pthread_self(), cpu_to_be_pinned);
#endif
...
void memcached_thread_init(int nthreads) {
	...
	int number_of_cpus = (int)sysconf(_SC_NPROCESSORS_ONLN);
	if (nthreads > number_of_cpus) {
		perror("number of threads > number of physical cores\n");
		exit(1);
	}
    for (i = 0; i < nthreads; i++) {
    	threads[i].cpu_to_be_pinned = i;
    	//printf("cpu_to_be_pinned: %d\n", threads[i].cpu_to_be_pinned);
    }
#endif
